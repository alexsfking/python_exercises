...
at most N times. Luckily, Alice and Bob are able to encode the motif as a
number. Can you help them to remove numbers such that their list contains each
...
def delete_nth(order:list[int],max_e:int) -> list[int]:
    ...
    for number in order:
        if number in occurence_dict:
            if occurence_dict[number] < max_e:
